...
public class QuestionApiController {
    ...
    public ResponseEntity createQuestion(@RequestBody Question question, Errors errors) {
        ...
        URI createdUri = linkTo(QuestionApiController.class).slash(savedQuestion.getId()).toUri();

        return ResponseEntity.created(createdUri).body(new Result(createQuestionResponse));
    }

    @PutMapping
    public ResponseEntity updateQuestion(@RequestBody Question question, Errors errors) {
        // 에러 체크
        if(errors.hasErrors()){
            return ResponseEntity.badRequest().body(errors);
        }
